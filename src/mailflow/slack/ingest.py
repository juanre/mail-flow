...
from mailflow.slack.client import SlackClient, SlackConfig
from mailflow.slack.markdown import render_transcript_md
from mailflow.slack.storage import (
    SlackArchivePaths,
    compute_latest_ts,
    write_json,
    write_markdown,
...
def token_path_for_entity(cfg: Config, entity: str) -> Path:
    ...
    # 2) XDG default: ~/.config/mailflow/slack/<entity>/user_token
    import os

    xdg = Path(os.environ.get("XDG_CONFIG_HOME", "~/.config")).expanduser()
    xdg_path = (xdg / "mailflow" / "slack" / entity / "user_token").resolve()
    if xdg_path.exists():
        return xdg_path
    # 3) Legacy fallback: ~/.mailflow/slack/<entity>/user_token
...
def _base_dir(cfg: Config) -> str:
    s = _get_slack_settings(cfg)
    return s.get("base_dir", "~/Documents/mailflow/slack")

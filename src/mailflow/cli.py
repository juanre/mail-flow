...
@cli.command(name="export-expenses")
@click.option("--archive", "-a", default="~/Archive", help="Archive base path")
@click.option("--output", "-o", required=True, help="Output CSV file path")
@click.option("--entity", "-e", default=None, help="Filter by entity (e.g., tsm)")
def export_expenses(archive, output, entity):
    """Export expenses from archive to CSV.

    Reads all sidecars with accounting.expense data and exports to a CSV file
    with columns: entity, workflow, expense_date, vendor, total_amount, currency,
    document_id, archive_path, and recommended fields.

    Each row includes document_id and archive_path for traceability.

    Example:
        mailflow export-expenses -a ~/Archive -o expenses.csv -e tsm
    """
    ...
    if not archive_path.exists():
        click.echo(f"Archive path does not exist: {archive_path}", err=True)
        sys.exit(1)
    ...
    click.echo(f"Exported {count} expense(s) to {output_path}")


@cli.command(name="export-xero")
@click.option("--archive", "-a", default="~/Archive", help="Archive base path")
@click.option("--output", "-o", required=True, help="Output CSV file path")
@click.option("--entity", "-e", default=None, help="Filter by entity (e.g., tsm)")
def export_xero(archive, output, entity):
    """Export expenses as Xero-compatible bills CSV.

    Reads all sidecars with accounting.expense data and exports to a CSV file
    formatted for Xero import with columns: ContactName, InvoiceNumber,
    InvoiceDate, DueDate, Description, Quantity, UnitAmount, AccountCode,
    TaxType, Reference.

    The Reference column includes the document_id for traceability.
    The Description column includes the archive_path for locating source files.

    Example:
        mailflow export-xero -a ~/Archive -o xero-bills.csv -e tsm
    """
    ...
    count = export_xero_bills_csv(
        archive_path=archive_path,
        output_path=output_path,
        entity=entity
    )

    click.echo(f"Exported {count} Xero bill(s) to {output_path}")

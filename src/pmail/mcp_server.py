"""pmail MCP Server (STDIO)

Exposes read-only tools over MCP so an MCP-enabled LLM can query the
pmail archive without changing local/Gmail flows.
...
Usage:
  uv run pmail-mcp  # starts STDIO server
"""
...
def _collect_base_dirs(config: Config) -> List[Path]:
    ...
    Sources:
      ...
      - Common defaults (~/Documents/pmail, ~/receipts)
    """
...
def create_server() -> MCPServer:
    server = MCPServer(name="pmail-mcp", version="1.0.0")
    config = Config()

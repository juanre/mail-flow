...
class SimilarityEngine:
    ...
    def __init__(self, config):
        ...
        self.recency_weight = config.settings["learning"]["recency_weight"]
    
    def calculate_similarity(self, email_features: Dict[str, Any], 
                           criteria_instance: 'CriteriaInstance') -> float:
        ...
        age_days = (datetime.now() - criteria_instance.timestamp).days
        recency_factor = math.exp(-age_days / 30.0)  # Decay over 30 days
        
        final_score = base_score * (1 - self.recency_weight + self.recency_weight * recency_factor)
    ...
    def rank_workflows(self, email_features: Dict[str, Any], 
                      criteria_instances: List['CriteriaInstance'],
                      top_n: int = 5) -> List[Tuple[str, float, List['CriteriaInstance']]]:
    ...
    def get_feature_explanation(self, email_features: Dict[str, Any],
                               criteria_instance: 'CriteriaInstance') -> Dict[str, str]:
        """Explain why an email matches a criteria instance"""
        
        explanations = {}
        
        if email_features.get("from_domain") == criteria_instance.email_features.get("from_domain"):
            explanations["from_domain"] = f"Same sender domain: {email_features.get('from_domain')}"
        ...
        if (email_features.get("has_pdf") and 
            criteria_instance.email_features.get("has_pdf")):

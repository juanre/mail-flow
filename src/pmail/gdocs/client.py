...
def _find_google_cred_paths() -> GoogleCredPaths:
    ...
    # legacy ~/.pmail
    legacy_dir = Path("~/.pmail").expanduser()
...
def _load_credentials(paths: GoogleCredPaths) -> Credentials:
    ...
    if creds is None:
        raise RuntimeError(
            "No Google credentials found. Place token.json at ~/.config/pmail/google/ or run pmail-setup."
        )

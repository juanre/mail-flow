...
def train_system(config, emails):
    ...
    for email_name, workflow, description in training:
        if email_name in emails:
            ...
            print(f"    Features: domain={email_data['features'].get('from_domain', 'N/A')}, "
...
def interactive_demo(config, emails):
    ...
    if "github_notification" in emails:
        ...
        email_data = extractor.extract(emails["github_notification"])
        
        # This will show the interactive UI
        selected_workflow = ui.select_workflow(email_data)
        
        if selected_workflow:
            print(f"\nEmail would be processed with workflow: {selected_workflow}")
        else:

...
class TestLearningProcess:
    ...
    def test_feature_importance(self, temp_pmail_dir, real_emails):
        ...
        invoice_pdf_rate = sum(1 for f in invoice_features if f.get("has_pdf", False)) / len(invoice_features)
        non_invoice_pdf_rate = sum(1 for f in non_invoice_features if f.get("has_pdf", False)) / len(non_invoice_features)
